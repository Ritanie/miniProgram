...
def display_calculator():
    ...
    if st.button("Submit"):
        ...
        st.subheader(f'Original Weight: {original_weight} lbs')
        if new_weight == 0 or package == '':
            st.write("Error. Please reselect your quantity number.")
        else:
            st.subheader(f'Selected Package Weight: {new_weight} lbs')
